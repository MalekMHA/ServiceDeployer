...
public class FeasibilitySlowTimeOverNodes {

	public static void main(String[] args) {
		
		String file = "FeasibilitySlowTimeOverNodes.csv";
		FileWriter fw;
			try {
				fw = new FileWriter(file);
				PrintWriter pw = new PrintWriter(fw, true); 
				
				for (int size = 1; size <= 51; size += 5) {
					float averageV = 0;
					int numberOfSolvables = 0;
					List<Long> times = new ArrayList<>();
					for (int sample = 0; sample < 100; sample++) {
						long startTime = System.nanoTime();
						AbstractGraph abstracted = CreateRandomNetwork(size);
						...
						List<Path> validPaths = Traversal.TraverseGraph(expandedGraph, abstracted.getProducer().getProcessNode(), abstracted.getConsumer().getProcessNode(), 15000, false, new ArrayList<Edge>(), 0);
						...
						if (validPaths.size() > 0) {
							...
							averageV += bestPath.getVulnerability();
							numberOfSolvables ++;
						}
						...
						times.add(importantTime);
						
						if (bestPathIndex >= 0) {					
							System.out.println("Node count: " + size + " | Sample: " + sample + " | Generated expanded graph in " + dExpansionTime + " | Traversed path in " + dTraversalTime + " | Found best path in " + dBestPathTime + " | Total time: " + importantTime + " | Number of valid paths: " + validPaths.size() + " | Best path vulnerability: " + validPaths.get(bestPathIndex).getVulnerability() + " | Best path: " );
						} else {
							System.out.println("Node count: " + size + " | Sample: " + sample + " | Generated expanded graph in " + dExpansionTime + " | Traversed path in " + dTraversalTime + " | Found best path in " + dBestPathTime + " | Total time: " + importantTime + " | Number of valid paths: " + validPaths.size() + " | Best path: None");
						}
					}
					
					averageV = averageV / numberOfSolvables;
					pw.print(size + ",");
					for (long t : times) {
						pw.print(t);
						pw.print(",");
...
//		}
	...
	private static AbstractGraph CreateRandomNetwork(int size){
		Random r = new Random();
...
//		}
	...
	private static void ConnectVertexToRandomVertexInGraph(AbstractGraph graph, AbstractNode node, List<AbstractNode> exceptions) {
		...
		float v = random.nextFloat() * 50 + 50;
